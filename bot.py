...
class MusicBot(discord.Client):
    def __init__(self, config_file=None, perms_file=None):
        ...
        if self.config._spotify:
            ...
            if not self.spotify.token:
                log.warning('Sua conta do Spotify não foi validada. veja seu Client ID e seu Client Secret '
                            'in the config file are correct. Disabling Spotify integration for this session.')
                self.config._spotify = False
            else:
                log.info('Seu Spotify foi autenticado com seu Client ID e Client Secret.')
    ...
    def owner_only(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if not orig_msg or orig_msg.author.id == self.config.owner_id:
                ...
                raise exceptions.PermissionsError("Apenas o yGustavo Pode usar esse comando.", expire_in=30)
    ...
    def dev_only(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if orig_msg.author.id in self.config.dev_ids:
                ...
                raise exceptions.PermissionsError("Comando apenas para Devs.", expire_in=30)
    ...
    def _setup_logging(self):
        if len(logging.getLogger(__package__).handlers) > 1:
            log.debug("Musica Skipada,tocando um novo batidão")
            return
    ...
    async def _join_startup_channels(self, channels, *, autosummon=True):
        ...
        def _autopause(player):
            if self._check_if_empty(player.voice_client.channel):
                log.info("A musica foi pausada pois não há ninguem no canal de Voz")
        ...
        for server in self.servers:
            ...
            if autosummon:
                ...
                if owner:
                    log.info("Dono encontrado em \"{}\"".format(owner.voice_channel.name))
                    channel_map[server] = owner.voice_channel

        for server, channel in channel_map.items():
            if server in joined_servers:
                log.info("Conectado em \"{}\", skipping".format(server.name))
                continue

            if channel and channel.type == discord.ChannelType.voice:
                log.info("Entrando em {0.server.name}/{0.name}".format(channel))

                chperms = channel.permissions_for(server.me)

                if not chperms.connect:
                    log.info("Não consigo entrar no canal \"{}\", me faltam permissões.".format(channel.name))
                    continue

                elif not chperms.speak:
                    log.info("Não vou entrar ai \"{}\", estou sem permissões para falar.".format(channel.name))
                    continue
                ...
                try:
                    ...
                    log.info("Conectado {0.server.name}/{0.name}".format(channel))
                    ...
                    log.debug("Erro ao entrar {0.server.name}/{0.name}".format(channel), exc_info=True)
                    log.error("Falha ao entrar {0.server.name}/{0.name}".format(channel))

            elif channel:
                log.warning("Não entrarei ai! {0.server.name}/{0.name}, Esse é um canal de texto.".format(channel))

            else:
                log.warning("Tipo de canal errado: {}".format(channel))
    ...
    async def _check_ignore_non_voice(self, msg):
        ...
        if not vc or vc == msg.author.voice_channel:
            ...
            raise exceptions.PermissionsError(
                "Você precisa estar conectado no canal de voz para usar esse comando! (%s)" % vc.name, expire_in=30)
    ...
    async def remove_from_autoplaylist(self, song_url:str, *, ex:Exception=None, delete_from_ap=False):
        ...
        async with self.aiolocks[_func_()]:
            ...
            log.info("Tirando musicas inativas da autoplaylist: %s" % song_url)

            with open(self.config.auto_playlist_removed_file, 'a', encoding='utf8') as f:
                f.write(
                    '# Musica removida {ctime}\n'
                    '# motivo: {ex}\n'
                    '{url}\n\n{sep}\n\n'.format(
                        ctime=time.ctime(),
                        ex=str(ex).replace('\n', '\n#' + ' ' * 10), # 10 spaces to line up with # motivo:
                        url=song_url,
                        sep='#' * 32
                ))

            if delete_from_ap:
                log.info("Atualizando a AutoPlaylist")
    ...
    async def join_voice_channel(self, channel):
        ...
        if getattr(channel, 'type', ChannelType.text) != ChannelType.voice:
            raise discord.InvalidArgument('Esse canal precisa ser um canal de voz.')

        server = channel.server

        if self.is_voice_connected(server):
            raise discord.ClientException('Já estou conectado em um canal de Voz nesse server')
        ...
        log.voicedebug("(%s) Aguardando a musica.", _func_())
        await self.ws.voice_state(server.id, channel.id)

        log.voicedebug("(%s) Esperando pelo ID da musica", _func_())
        ...
        log.voicedebug("(%s) Esperando pela Data da Voz :/", _func_())
        ...
        try:
            log.voicedebug("(%s) Conectando...", _func_())
            with aiohttp.Timeout(15):
                await voice.connect()

        except asyncio.TimeoutError as e:
            log.voicedebug("(%s) Conexão falhou, desconectando", _func_())
            try:
        ...
        log.voicedebug("(%s) Sucesso! Conectando", _func_())
    ...
    async def get_voice_client(self, channel: discord.Channel):
        ...
        if getattr(channel, 'type', ChannelType.text) != ChannelType.voice:
            raise AttributeError('Esse canal tenque ser um canal de voz!')
        ...
        async with self.aiolocks[_func_() + ':' + channel.server.id]:
            ...
            for attempt in range(1, tries+1):
                log.debug("Conectando em {} para {}".format(attempt, channel.name))
                t0 = time.time()
                ...
                try:
                    ...
                    log.warning("Falha ao conectar, tentando reconectar ({}/{})".format(attempt, tries))
                    ...
                    log.exception("Erro desconhecido ao entrar no canal de voz")

                await asyncio.sleep(0.5)

            if not vc:
                log.critical("Não estou conseguindo entrar nesse canal de voz! irei tentar novamente...")
                await self.restart()
    ...
    async def reconnect_voice_client(self, server, *, sleep=0.1, channel=None):
        log.debug("Reconectando em \"{}\"{}".format(
        ...
        async with self.aiolocks[_func_() + ':' + server.id]:
            ...
            if player and player.is_playing:
                log.voicedebug("(%s) Pausado", _func_())

                player.pause()
                _paused = True

            log.voicedebug("(%s) Desconectando~", _func_())
            ...
            if sleep:
                log.voicedebug("(%s) Dormindo por %s", _func_(), sleep)
                await asyncio.sleep(sleep)

            if player:
                log.voicedebug("(%s) Entrando no canal de voz", _func_())
                ...
                log.voicedebug("(%s) Mudando de canal de voz~", _func_())
                await player.reload_voice(new_vc)

                if player.is_paused and _paused:
                    log.voicedebug("Voltando a tocar o batidão")
                    player.resume()

        log.debug("Reconectei em \"{}\"{}".format(
            server, ' to "{}"'.format(channel.name) if channel else ''))
    ...
    async def get_player(self, channel, create=False, *, deserialize=False) -> MusicPlayer:
        ...
        async with self.aiolocks[_func_() + ':' + server.id]:
            if deserialize:
                ...
                if player:
                    log.debug("Player criado via desserialização para o servidor %s com %s membros", server.id, len(player.playlist))
            ...
            if server.id not in self.players:
                if not create:
                    raise exceptions.CommandError(
                        ...
                        'Use %ssummon para tocar uns batidão.' % self.config.command_prefix)
            ...
            async with self.aiolocks[self.reconnect_voice_client.__name__ + ':' + server.id]:
                if self.players[server.id].voice_client not in self.voice_clients:
                    log.debug("Reconexão requirida em {}".format(server.name))
    ...
    async def on_player_play(self, player, entry):
        ...
        if channel and author:
            ...
            if author not in player.voice_client.channel.voice_members and author_perms.skip_when_absent:
                newmsg = 'Skipando a proxima musica em `%s`: `%s` adicionado por `%s` que não se encontra no canal de voz' % (
                    player.voice_client.channel.name, entry.title, entry.meta['author'].name)
                player.skip()
            elif self.config.now_playing_mentions:
                newmsg = '%s - Sua musica está tocando `%s` agora está tocando em `%s`!' % (
                    entry.meta['author'].mention, entry.title, player.voice_client.channel.name)
            else:
                newmsg = 'Tocando agora `%s`: `%s` adicionado por `%s`' % (
                    player.voice_client.channel.name, entry.title, entry.meta['author'].name)
    ...
    async def on_player_finished_playing(self, player, **_):
        def _autopause(player):
            if self._check_if_empty(player.voice_client.channel):
                log.info("Todos desconectaram do canal de voz, pausando a musica.")
        ...
        if not player.playlist.entries and not player.current_entry and self.config.auto_playlist:
            if not player.autoplaylist:
                if not self.autoplaylist:
                    ...
                    log.warning("Sem musicas na auto playlist.")
                    self.config.auto_playlist = False
                else:
                    log.debug("Não há nada na autoplaylist. Colocando alguns batidões..")
...
#######################################################################################################################
        ...
        if skips_remaining <= 0:
            ...
            return Response(
                self.str.get('cmd-skip-reply-skipped-1', 'Your skip for `{0}` was acknowledged.\nThe vote to skip has been passed.{1}').format(
                    ...
                    self.str.get('cmd-skip-reply-skipped-2', ' Proxima musica vindo ai!') if player.playlist.peek() else ''
                ),
                reply=True,
                delete_after=20
            )
            ...
            return Response(
                self.str.get('cmd-skip-reply-voted-1', 'Seu skip `{0}` foi aclamado.\n**{1}** mais {2} para skipar esse som.').format(
                    player.current_entry.title,
                    skips_remaining,
    ...
    async def cmd_queue(self, channel, player):
        ...
        if player.current_entry:
            ...
            if player.current_entry.meta.get('channel', False) and player.current_entry.meta.get('author', False):
                lines.append(self.str.get('cmd-queue-playing-author', "Tocando agora: `{0}` adicionado por `{1}` {2}\n").format(
                    player.current_entry.title, player.current_entry.meta['author'].name, prog_str))
            else:
                lines.append(self.str.get('cmd-queue-playing-noauthor', "Tocando agora: `{0}` {1}\n").format(player.current_entry.title, prog_str))


        for i, item in enumerate(player.playlist, 1):
            if item.meta.get('channel', False) and item.meta.get('author', False):
                nextline = self.str.get('cmd-queue-entry-author', '{0} -- `{1}` por `{2}`').format(i, item.title, item.meta['author'].name).strip()
            else:
    ...
    async def cmd_pldump(self, channel, song_url):
        ...
        if not exfunc:
            raise exceptions.CommandError("Não consegui verificar o url.", expire_in=25)
        ...
        with BytesIO() as fcontent:
            ...
            fcontent.seek(0)
            await self.send_file(channel, fcontent, filename='playlist.txt', content="Aqui está o deposito de url <%s>" % song_url)
    ...
    async def cmd_listids(self, server, author, leftover_args, cat='all'):
        ...
        return Response("Mande a mensagem com a lista de IDs.", delete_after=20)
    ...
    async def cmd_perms(self, author, user_mentions, channel, server, permissions):
        ...
        lines = ['Permissões de comando em %s\n' % server.name, '```', '```']
    ...
    async def cmd_setname(self, leftover_args, name):
        ...
        try:
            ...
            raise exceptions.CommandError(
                "Falhou, você não pode mudar meu nome toda hora -.-  "
                "Lembra-se Voce pode mudar o meu nome apenas a cada 1h.")

        except Exception as e:
            raise exceptions.CommandError(e, expire_in=20)

        return Response("O nickname do Bot foi setado como **{0}**".format(name), delete_after=20)
    ...
    async def cmd_setnick(self, server, channel, leftover_args, nick):
        ...
        if not channel.permissions_for(server.me).change_nickname:
            raise exceptions.CommandError("Você não pode usar esse comando.")
        ...
        return Response("Nickname do bot foi setado como `{0}`".format(nick), delete_after=20)
    ...
    async def cmd_setavatar(self, message, url=None):
        ...
        if message.attachments:
            ...
            raise exceptions.CommandError("Você deve colocar um link ou uma Imagem.", expire_in=20)
        ...
        try:
            ...
            raise exceptions.CommandError("Não consegui mudar o avatar : {}".format(e), expire_in=20)

        return Response("Avatar novosetado ^^.", delete_after=20)


    async def cmd_disconnect(self, server):
        await self.disconnect_voice_client(server)
        return Response("Desconectado do `{0.name}`".format(server), delete_after=20)

    async def cmd_restart(self, channel):
        await self.safe_send_message(channel, "\N{WAVING HAND SIGN} Reiniciando. agora você terá um bot com as novas Updates! "
            "Você precisará reiniciar o bot antes de usar esse comando.")
    ...
    async def cmd_leaveserver(self, val, leftover_args):
        ...
        if t is None:
            ...
            if t is None:
                raise exceptions.CommandError('Nenhum server encontrado com esse ID `{0}`'.format(val))
        await self.leave_server(t)
        return Response('Sai do servidor: `{0.name}` (Owner: `{0.owner.name}`, ID: `{0.id}`)'.format(t))

    @dev_only
    async def cmd_breakpoint(self, message):
        log.critical("Ativando o modo de Debug")
        return
    ...
    async def on_message(self, message):
        ...
        if message.author == self.user:
            log.warning("Ignorando comandos de mim mesmo ({})".format(message.content))
            return
        ...
        if message.channel.is_private:
            if not (message.author.id == self.config.owner_id and command == 'joinserver'):
                await self.send_message(message.channel, 'Você não pode usar esse bot nas Dm Baka.')
                return

        if message.author.id in self.blacklist and message.author.id != self.config.owner_id:
            log.warning("Usuario na Lista negra: {0.id}/{0!s} ({1})".format(message.author, command))
            return
        ...
        try:
            ...
            if message.author.id != self.config.owner_id:
                if user_permissions.command_whitelist and command not in user_permissions.command_whitelist:
                    raise exceptions.PermissionsError(
                        "Esse comando está desativado nesse grupo ({}).".format(user_permissions.name),
                        expire_in=20)

                elif user_permissions.command_blacklist and command in user_permissions.command_blacklist:
                    raise exceptions.PermissionsError(
                        "Esse comando foi desativado nesse grupo ({}).".format(user_permissions.name),
                        expire_in=20)
            ...
            log.error("Erro no {0}: {1.__class__.__name__}: {1.message}".format(command, e), exc_info=True)
    ...
    async def on_voice_state_update(self, before, after):
        ...
        if state.broken:
            log.voicedebug("Chat de voz quebrado")
            return

        if state.resuming:
            log.debug("Conexão está de volta! resumindo em {0.server.name}/{0.name}".format(state.voice_channel))

        if not state.changes:
            log.voicedebug("Chat de voz vazio")
            return # Session id change, pointless event

        ################################

        log.voicedebug("Status de voz alterado por {mem.id}/{mem!s} no {ser.name}/{vch.name} -> {dif}".format(
            mem = state.member,
            ser = state.server,
        ...
        if state.joining and state.empty() and player.is_playing:
            log.info(autopause_msg.format(
                ...
                reason = "(Entrando em chat de voz vazio)"
            ).strip())
    ...
    async def on_server_join(self, server:discord.Server):
        log.info("Bot entrou no servidor: {}".format(server.name))

        if not self.user.bot:
            alertmsg = "<@{uid}> Olá eu sou um bot de musica."
        ...
        log.debug("Criando uma Data pro servidor %s", server.id)
        pathlib.Path('data/%s/' % server.id).mkdir(exist_ok=True)

    async def on_server_remove(self, server: discord.Server):
        log.info("Bot foi kickado do servidor: {}".format(server.name))
    ...
    async def on_server_available(self, server: discord.Server):
        ...
        log.debug("Server \"{}\" voltou a ativa!.".format(server.name))
        ...
        if player and player.is_paused:
            ...
            if av_paused:
                log.debug("Resumindo o player em \"{}\" .".format(server.name))
                self.server_specific_data[server]['availability_paused'] = False
                player.resume()


    async def on_server_unavailable(self, server: discord.Server):
        log.debug("Server \"{}\" está inativo.".format(server.name))

        player = self.get_player_in(server)

        if player and player.is_playing:
            log.debug("Pausando o player em \"{}\" por inatividade.".format(server.name))
